<?php
	...
	class clienteControlador extends mainModel{
        ...
        public function registrar_cliente_controlador(){
            ...
            if(isset($_SESSION['cargo_sto']) && ($_SESSION['cargo_sto']=="Administrador" || $_SESSION['cargo_sto']=="Usuario")){
            ...
            $municipio=mainModel::limpiar_cadena($_POST['cliente_municipio_reg']);
            $departamento=mainModel::limpiar_cadena($_POST['cliente_departamento_reg']);
            ...
            if($nombre=="" || $apellido=="" || $telefono=="" || $genero=="" || $municipio=="" || $departamento=="" || $direccion=="" || $email=="" || $clave_1=="" || $clave_2==""){
                $alerta=[
					"Alerta"=>"simple",
            ...
            if(mainModel::verificar_datos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,29}",$municipio)){
                $alerta=[
                    "Alerta"=>"simple",
                    "Titulo"=>"Formato no valido",
                    "Texto"=>"Municipio no coincide con el formato solicitado",
                    "Icon"=>"error",
                    "TxtBtn"=>"Aceptar"
                ];
                echo json_encode($alerta);
            ...
            if(mainModel::verificar_datos("[a-zA-ZáéíóúÁÉÍÓÚñÑ ]{4,29}",$departamento)){
                $alerta=[
                    "Alerta"=>"simple",
                    "Titulo"=>"Formato no valido",
                    "Texto"=>"Departamento no coincide con el formato solicitado",
                    "Icon"=>"error",
                    "TxtBtn"=>"Aceptar"
                ];
                echo json_encode($alerta);
        ...
        public function paginador_cliente_controlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
				$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM cliente WHERE (cliente_nombre LIKE '%$busqueda%' OR cliente_apellido LIKE '%$busqueda%' OR cliente_email LIKE '%$busqueda%' OR cliente_municipio LIKE '%$busqueda%' OR cliente_departamento LIKE '%$busqueda%') ORDER BY cliente_nombre ASC LIMIT $inicio,$registros";
			}else{
		...
		public function actualizar_cliente_controlador(){
            ...
            $municipio=mainModel::limpiar_cadena($_POST['cliente_municipio_up']);
            $departamento=mainModel::limpiar_cadena($_POST['cliente_departamento_up']);
            ...
            /*-- Comprobando campos vacios - Checking empty fields --*/
            if($nombre=="" || $apellido=="" || $telefono=="" || $genero=="" || $municipio=="" || $departamento=="" || $direccion=="" || $email=="" || $usuario_usuario=="" || $usuario_clave==""){
                $alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"No has llenado todos los campos que son obligatorios",
